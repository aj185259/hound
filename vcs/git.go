...
type GitDriver struct {
	...
	githubTokenEnvVar string `json:"github-token-env-var"`
}
...
// This function inserts the GitHub Personal Access Token if provided
func urlWithGithubToken(url string, envVar string) string {
	// Clone will fail as unauthorized
	githubToken, found := os.LookupEnv(envVar)
	if !found {
		return url
	}

	urlWithToken := strings.ReplaceAll(
		url,
		"https://",
		fmt.Sprintf("https://%s@", githubToken),
	)
...
func (g *GitDriver) Clone(dir, url string) (string, error) {
	...
	cloneUrl := urlWithGithubToken(url, g.githubTokenEnvVar)
	cmd := exec.Command(
		"git",
		"clone",
		"--depth", "1",
